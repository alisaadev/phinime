...
export type AnimeDetail = {
  ...
  genreList: Genre[];
  batchList?: { title: string; batchId: string; href: string; samehadakuUrl: string }[];
  episodeList?: { title: number; episodeId: string; href: string; samehadakuUrl: string }[];
};
...
export type AnimeListGroup = {
  startWith: string;
  animeList: { title: string; animeId: string; href: string; samehadakuUrl: string }[];
};
...
export const getAnimeList = () =>
  cacheOrFetch<{ list: AnimeListGroup[] }>(
    "anime_list",
    () => fetchApi("/list")
  );
...
export const getBatchList = (page = 1) =>
  cacheOrFetch<{ batchList: BatchListItem[] }>(
    `batch_list:page:${page}`,
